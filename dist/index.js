...
const getLatestPublishedVersion = async ({ packageName, token, org }) => {
    const versions = await (0, node_fetch_1.default)(`https://api.github.com/orgs/${org}/packages/npm/${packageName}/versions`, {
        headers: {
...
async function run() {
    try {
        ...
        const contextPath = path_1.default.join(cwd, packagePath);
        // Get the context package package.json
        const packageJson = JSON.parse(fs_1.default.readFileSync(path_1.default.join(contextPath, 'package.json'), 'utf8'));
        ...
        console.log('build-publish-monorepo-package-action', packageJson.name, cwd, packageJson.version, published);
